...
export CNRANCHER_VERSION=$( curl -L -u ${token} -s https://api.github.com/repos/cnrancher/pandaria/git/refs/tags | jq -r .[].ref | awk -F/ '{print $3}' | grep v | awk -Fv '{print $2}' | grep -v -E 'rc|2.2|2.3|2.4|2.5'|sort -r );
...
pull_tag_push_image(){
    ...
    if [[ ${tag} != '' ]]; then
        #重命名镜像
        echo "TAG IMAGES ${imgs} ${registry}/${projects}/${repositories}:${tag}";
        docker tag ${imgs} ${registry}/${projects}/${repositories}:${tag};
        #上传镜像
        echo "PUSH IMAGES ${imgs} ${registry}/${projects}/${repositories}:${tag}";
        docker push ${registry}/${projects}/${repositories}:${tag};
        #删除旧镜像
        echo "DELETE LOCAL IMAGES ${imgs} ${registry}/${projects}/${repositories}:${tag}";
        docker rmi ${imgs} ${registry}/${projects}/${repositories}:${tag} -f;
    else
        #重命名镜像
        echo "TAG IMAGES ${imgs} ${registry}/${projects}/${repositories}";
        docker tag ${imgs} ${registry}/${projects}/${repositories};
        #上传镜像
        echo "PUSH IMAGES ${imgs} ${registry}/${projects}/${repositories}";
        docker push ${registry}/${projects}/${repositories};
        #删除旧镜像
        echo "DELETE LOCAL IMAGES ${imgs} ${registry}/${projects}/${repositories}";
        docker rmi ${imgs} ${registry}/${projects}/${repositories} -f;
    fi
}
        ...
        if [ ${n} -eq 0 ]; then
            ...
            if [[ ${tag} != '' ]]; then
                ...
                if [[ ${CHECK_STATUS_CODE} == '0' ]]; then
                    ...
                    pull_tag_push_image $imgs $registry $projects $repositories $tag;
                fi
            else
                docker pull ${imgs};
        ...
        # 如果镜像名中有一个 /，那么 / 左侧为项目名，右侧为镜像名和 tag
            ...
            if [[ ${tag} != '' ]]; then
                ...
                if [[ ${CHECK_STATUS_CODE} == '0' ]]; then
                    ...
                    if [[ ${projects} == 'prime' ]]; then
                        docker pull cnrancher/${repositories}:${tag};
                        docker tag cnrancher/${repositories}:${tag} ${imgs};
                ...
                if [[ ${projects} == 'prime' ]]; then
                    docker pull cnrancher/${repositories};
                    docker tag cnrancher/${repositories} ${imgs};
