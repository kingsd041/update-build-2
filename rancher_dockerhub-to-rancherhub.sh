...
export RANCHER_VERSION=$( curl -u ${token} -s https://api.github.com/repos/rancher/rancher/git/refs/tags | jq -r .[].ref | awk -F/ '{print $3}' | grep v | grep -v windows | awk -Fv '{print $2}' | grep -v [a-z] | grep -vE '^0.|^1.|^2.0|^2.1|^2.2|^2.3|^2.4|^2.5'|awk -v num=3 -F"." 'BEGIN{i=1}{if(tmp==$1"."$2){i=i+1}else{tmp=$1"."$2;i=1};arr[$0]=i;arrMax[$1"."$2]=i}END{for(var in arr){split(var,arrTmp,".");if(arr[var]>=(arrMax[arrTmp[1]"."arrTmp[2]]-num)){print var}}}'|sort -r )
...
cat rancher-images-all.txt | grep -vE 'documentation_url|{|}|Found|Not|AuthenticationFailed|AuthenticationErrorDetail|arm64|windows|404' | sed 's/Not Founddocker/docker/' | sed 's#registry.rancher.cn/##' > rancher-images-all-new.txt
