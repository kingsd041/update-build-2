...
##
# rke2 镜像
export rke2_version=$( curl -u ${token} -L -s https://api.github.com/repos/rancher/rke2/git/refs/tags | jq -r .[].ref | awk -F/ '{print $3}' | grep v | awk -Fv '{print $2}' | grep -v -E 'alpha|rc|beta|^1.18|^1.19|^1.20|^1.21|^1.22|^1.23|^1.24|^1.25' | awk -v num=3 -F"." 'BEGIN{i=1}{if(tmp==$1"."$2){i=i+1}else{tmp=$1"."$2;i=1};arr[$0]=i;arrMax[$1"."$2]=i}END{for(var in arr){split(var,arrTmp,".");if(arr[var]>=(arrMax[arrTmp[1]"."arrTmp[2]]-num)){print var}}}'|sort -r  )
        ...
        if [[ ${CHECK_STATUS_CODE} == '0' ]]; then
            ...
            if [[ -n "${global_namespace}" ]]; then
                ...
                if [ ${n} -eq 0 ]; then
                    ...
                    #上传镜像
                    echo "PUSH IMAGES ${registry}/${namespace}/${img_tag}"
                    docker push ${registry}/${namespace}/${img_tag} 2>&1 > /dev/null
