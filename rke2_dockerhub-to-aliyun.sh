...
# rke2 镜像
export rke2_version=$( curl -L -u $token -s https://api.github.com/repos/rancher/rke2/git/refs/tags | jq -r .[].ref | awk -F/ '{print $3}' | grep v | awk -Fv '{print $2}' | grep -v -E 'alpha|rc|beta|^1.18|^1.19|^1.20|^1.21|^1.22|^1.23|^1.24|^1.25|^1.26' | awk -v num=3 -F"." 'BEGIN{i=1}{if(tmp==$1"."$2){i=i+1}else{tmp=$1"."$2;i=1};arr[$0]=i;arrMax[$1"."$2]=i}END{for(var in arr){split(var,arrTmp,".");if(arr[var]>=(arrMax[arrTmp[1]"."arrTmp[2]]-num)){print var}}}'|sort -r )
for ver in $( echo "${rke2_version}" );
do
    browser_download_url_list=$( curl -LSs https://api.github.com/repos/rancher/rke2/releases/tags/v${ver} | jq ".assets[].browser_download_url" -r | grep txt | grep -v windows | grep -v sha256sum )
...
# 生成 hangar 同步的执行脚本
cat >sync-rke2-to-aliyun.sh <<EOL
#!/bin/bash
    # 添加调试信息
...
docker run --rm -v $(pwd):/hangar --network=host cnrancher/hangar:latest bash sync-rke2-to-aliyun.sh 